...
<%
	...
	System.out.println("");
	System.out.println("WorkGroups/_get_head_unassigned.jsp: ");
	...
	try {
		sql = "begin " + ReadDBProperties.getProperty("conn_uname")+ ".api_work_group_admin.get_wg_nomanage_admin(?,?,?,?,?,?,?,?,?); end;";
...
<%if ("".equals(view_search)) {%>
	...
	<i class="fa fa-square-o"></i>
		<h3 class="box-title" title="Недоделени раководители">Недоделени раководители&nbsp;&nbsp;<span class="label label-success pull-right"><%=vkupno%></span></h3>
	
		<%if (!"0".equals(vkupno)) {%>
		<div class="box-tools pull-right">
		<a class="btn btn-primary btn-xs" data-toggle="modal" data-target="#ModalView" onClick="openModalSearch('<%=count%>','head')">пребарај</a>
<!-- 			<a class="btn btn-success btn-xs" -->
<%-- 				onClick="insHeadWorkGroup('<%=id%>','','<%=count%>')" --%>
<!-- 				title="Додели ги сите недоделени раководители"><i -->
<!-- 				class="fa  fa-hand-o-left"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; -->
				 <a
				class="btn btn-box-tool" data-widget="collapse"
				id="get_head_unassigned_but"> <i class="fa fa-plus"></i>
			</a>
		</div>
		<%}%>
...
%>
	...
	%>
		...
		<tr>
			...
			<td width="5%">
			<%if ("".equals(view_search)) {%>
			<a class="btn btn-success btn-xs" title="<%=SIF%>" onClick="insHeadWorkGroup('<%=id%>','<%=SIF%>','0')"><i class="fa fa-long-arrow-left"></i></a>
			<%}%>
			<%if (!"".equals(view_search)) {%>
			<a class="btn btn-success btn-xs" title="<%=SIF%>"  data-dismiss="modal" onClick="insHeadWorkGroup('<%=id%>','<%=SIF%>','0')"><i class="fa fa-long-arrow-left"></i></a>
			<%}%>
			</td>
		</tr>
		<%
	...
	<%if ("".equals(view_search)&&"0".equals(count)&&(vkupno_int>m)) {%>
		<div class="box-footer pull-right">
			<a class="btn btn-default btn-xs" onClick="getHeadWorkGroup('<%=id%>','unassig','1')" title="прикажи ги сите">прикажи ги сите</a>
		</div>
	<%}%>
	<!-- =============================================================================================================================== -->
	<%
