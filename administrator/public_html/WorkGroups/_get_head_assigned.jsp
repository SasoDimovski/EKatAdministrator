...
<%
	...
	System.out.println("");
	System.out.println("WorkGroups/_get_head_assigned.jsp: ");
  ...
  int n =1;
	...
	try {
		sql = "begin " + ReadDBProperties.getProperty("conn_uname")+ ".api_work_group_admin.get_wg_manage_admin(?,?,?,?,?); end;";
...
<%if ("1".equals(pregled)) {%>	<button type="button" class="close" data-dismiss="modal">&times;</button><%}%>
	...
	<h3 class="box-title" title="Доделени раководители">
		Доделени раководители&nbsp;&nbsp;<span class="label label-success pull-right"><%=vkupno%></span>
	</h3>
	...
	<div class="box-tools pull-right">
		<a class="btn btn-success btn-xs"
			onClick="delHeadWorkGroup('<%=id%>','','0')"
			title="Избриши ги сите доделени раководители"><i
			class="fa fa-hand-o-right"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a
			class="btn btn-box-tool" data-widget="collapse"
			id="get_head_assigned_but"> <i class="fa fa-plus"></i>
		</a>
	</div>
	<%}%>
...
<div class="box-body no-padding table-responsive"   id="get_head_assigned_sub">
	<%
			if (!"0".equals(vkupno)) {
...
%>
	...
	%>
		...
		<tr>
			...
			<td width="5%">
				<%if (!"1".equals(pregled)) {%>
			  <a class="btn btn-success btn-xs" title="<%=SIF%>" onClick="delHeadWorkGroup('<%=id%>','<%=SIF%>','0')"><i class="fa fa-long-arrow-right"></i></a>
			  		<%}%>
			</td>
		</tr>
		<%
	...
	<%if ("0".equals(count)&&(vkupno_int>m)) {%>
		<div class="box-footer pull-right">
			<a class="btn btn-default btn-xs" onClick="getHeadWorkGroup('<%=id%>','assig','1')" title="прикажи ги сите">прикажи ги сите</a>
		</div>
	<%}%>
	<!-- =============================================================================================================================== -->
	<%
