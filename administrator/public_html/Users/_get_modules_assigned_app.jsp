...
<%
	...
	System.out.println("");
	System.out.println("Users/_get_modules_assigned_app.jsp: ");
...
<%if ("1".equals(pregled)) {%>	<button type="button" class="close" data-dismiss="modal">&times;</button><%}%>
	...
	<h5 class="box-title"  title="Доделени привилегии, апликации">
		Доделени привилегии&nbsp;&nbsp;<span class="label label-success pull-right"><%=vkupno%></span>
	...
	<div class="box-tools pull-right">
		<a class="btn btn-success btn-xs"
			onClick="delModulesUser('<%=sif%>','','','get_modules_assigned_sub','<%=prava%>')"
...
%>
	...
	<%if (!"1".equals(pregled)) {%>
				<a class="btn btn-btn-xs"
					onClick="delModulesUser('<%=sif%>','<%=APL_SIF%>','','get_modules_assigned_app<%=APL_SIF%>','<%=prava%>')"
