...
<%
	...
	System.out.println("");
	System.out.println("Groups/_get_modules_search.jsp: ");
...
<%}%>
	...
	<%if (!"0".equals(NEXT_LEVEL)) {%>
		<button style="cursor:pointer;padding-left:2px;padding-right:2px" onClick="getModulesApp_search('<%=app_sif%>','<%=SIF%>','<%=SIF%>')"  data-toggle="collapse"
			data-target="#<%=SIF%>"  id="2<%=MOD_SIF%>"><b title="Име на привилегија: <%=NAZIV%>"><%=NAZIV_SHORT%></b> <span class="text-green"  title="Датотека"><%=DATOTEKA%></span>
			 <span class="text-red"><%=TIP_MODUL_NAZIV%></span>
			  <span class="text-yellow" title="ID"><%=SIF%></span>
			   ...
			   <small onClick="insetModules_search('<%=SIF%>','<%=NAZIV%>')"  title="" class="linkStrelka" data-dismiss="modal"><i class="fa fa-arrow-right "></i></small>
	<%}else{%>
