...
<%
	...
	System.out.println("");
	System.out.println("Groups/_get_modules_app.jsp: ");
	...
	try {
		sql = "begin " + ReadDBProperties.getProperty("conn_uname")+ ".api_codes.get_group_moduls(?,?,?,?,?,?,?,?); end;";
...
<%-- 			<small id="1exp<%=MOD_SIF%><%=prava%>"><i class="fa fa-plus"></i></small> --%>
	<%}else{%>
	<b title="Име на привилегија"><%=MOD_NAZIV%></b> <span class="text-green" title="Датотека"><%=MOD_DATOTEKA%></span> <span class="text-yellow" title="ID"><%=MOD_SIF%></span>
	<%}%>
	&nbsp;
	<%if ("0".equals(prava)&&!"1".equals(pregled)) {%>
	<small onClick="insModulesGroup('<%=sif%>','<%=APL_SIF%>','<%=MOD_SIF%>','<%=MOD_SIF%><%=prava%>','<%=prava%>')"  title="Додели ја привилегијата '<%=MOD_DATOTEKA%>, <%=MOD_NAZIV%>' и сите нејзини подпривилегии" class="linkStrelka"><i class="fa fa-arrow-left "></i></small>
	<%}%>
	<%if ("1".equals(prava)&&!"1".equals(pregled)) {%>
	<small onClick="delModulesGroup('<%=sif%>','<%=APL_SIF%>','<%=MOD_SIF%>','<%=MOD_SIF%><%=prava%>','<%=prava%>')"  title="Избриши ја привилегијата: '<%=MOD_DATOTEKA%>, <%=MOD_NAZIV%>' и сите нејзини подпривилегии" class="linkStrelka"><i class="fa fa-arrow-right "></i></small>
	<%}%>
	</h5>
