...
<%
	...
	System.out.println("");
	System.out.println("Statistic/_get_modules_search.jsp: ");
...
<%
		while (rs.next()) {
			...
			NAZIV_SHORT= rs.getString("NAZIV");if (NAZIV_SHORT==null){NAZIV_SHORT="";}
			int length = NAZIV.length( );
			if (length>40){NAZIV_SHORT = NAZIV.substring(0,40) + "...";}
...
<%}%>
	...
	<%if (!"0".equals(NEXT_LEVEL)) {%>
		<button style="cursor:pointer;padding-left:2px;padding-right:2px" onClick="getModulesApp_search_stat('<%=app_sif%>','<%=SIF%>','<%=SIF%>')"  data-toggle="collapse"
			data-target="#<%=SIF%>"  id="2<%=MOD_SIF%>"><b title="Име на привилегија: <%=NAZIV%>"><%=NAZIV_SHORT%></b> <span class="text-green"  title="Датотека"><%=DATOTEKA%></span>
			 <span class="text-red"><%=TIP_MODUL_NAZIV%></span>
			  <span class="text-yellow" title="ID"><%=SIF%></span>
			   <span class="text-aqua" title="ID Parent"><%=MOD_SIF%></span>
			   </button>&nbsp;&nbsp;
			   <small onClick="insetModules_search_stat('<%=SIF%>','<%=NAZIV%>')"  title="" class="linkStrelka" data-dismiss="modal"><i class="fa fa-arrow-right "></i></small>
	<%}else{%>
	<b title="Име на привилегија: <%=NAZIV%>"><%=NAZIV_SHORT%></b> 
	<span class="text-green"  title="Датотека"><%=DATOTEKA%></span> 
	<span class="text-red" title="Тип на привилегија"><%=TIP_MODUL_NAZIV%></span> 
	<span class="text-yellow" title="ID"><%=SIF%></span> 
	<span class="text-aqua" title="ID Parent"><%=MOD_SIF%></span>&nbsp;&nbsp;
		<small onClick="insetModules_search_stat('<%=SIF%>','<%=NAZIV%>')"  title="" class="linkStrelka" data-dismiss="modal"><i class="fa fa-arrow-right "></i></small>
	<%}%>
